...
| **Конструктор курсов** | Визуальный конструктор с drag-and-drop для создания курсов | ✅ Готов |
...
| **Админка** | Современная React админ-панель с дашбордом, аналитикой, экспортом и ролями | ✅ Готов |
...
- **Конструктор курсов**: Визуальный drag-and-drop редактор с 9 типами блоков контента
- **Модульная архитектура**: Курсы состоят из страниц и переиспользуемых блоков
- **Шаблоны контента**: Система шаблонов для быстрого создания курсов
- **Аналитика блоков**: Статистика эффективности различных типов контента
- **Система напоминаний**: Управление уходом за питомцами с повторяющимися событиями
- **Современная админ-панель**: React-based интерфейс с Chart.js аналитикой, экспортом данных и ролевой системой
- **Интерактивная аналитика**: Drill-down графики, real-time обновления, кастомные дашборды
- **Система экспорта**: CSV, Excel, PDF, JSON экспорт с фильтрацией и настройками
...
| Rich Text Editor | @tiptap | 2.1.12 |
...
Для тестирования системы используются команды Django management:

```bash
# Загрузка товаров и курсов
python manage.py load_test_data

# Создание пользователей с питомцами
python manage.py create_test_users --count=10
```

### Созданные тестовые данные

**Staff пользователи (администраторы):**
...
- **Каждый пользователь имеет 11-20 заказов** с товарами
- Каждый имеет записи на курсы с разным прогрессом

**Статистика тестовых данных:**
- 👥 **10 основных пользователей** с полными профилями
- 🐕 **16 питомцев** (собаки и кошки разных пород, вес, аллергии)
- 📦 **195 заказов** у тестовых пользователей (11-20 заказов на пользователя)
- 🎓 **11 записей** на курсы (с разным прогрессом прохождения)
- 💰 **Разнообразные суммы** заказов (от сотен до тысяч рублей)

**Тестовые пользователи с заказами:**
- `alexandr.ivanov@test.ru` - Александр Иванов (11 заказов)
- `maria.petrov@test.ru` - Мария Петрова (12 заказов)
- `dmitry.sidorov@test.ru` - Дмитрий Сидоров (16 заказов)
- `elena.kuznetsova@test.ru` - Елена Кузнецова (16 заказов)
- `andrey.vasilev@test.ru` - Андрей Васильев (18 заказов)
- `olga.morozova@test.ru` - Ольга Морозова (14 заказов)
- `sergey.novikov@test.ru` - Сергей Новиков (20 заказов)

Все тестовые пользователи имеют пароль `test123` для входа.

#### Доступ к React админке

**URL:** `http://192.168.1.11:5199/admin-panel/`

**Тестовые аккаунты:**
- `testadmin@test.ru` / `test123` (суперпользователь)
- `admin@pitometsplus.ru` / `admin123` (администратор)
- `manager@pitometsplus.ru` / `manager123` (менеджер)

**Особенности:**
- Все данные загружаются из реальной базы данных
- Полная интеграция с Django ORM через REST API
- Навигация между разделами работает
- Интерфейс адаптивный (десктоп + мобильная версия)

#### Доступные таблицы данных

| Таблица | Данные из базы | Функции |
|---------|----------------|---------|
| **👥 Пользователи** | Реальные пользователи с профилями | Просмотр, фильтры по email/имени/статусу |
| **🐾 Питомцы** | Все питомцы из базы данных | Фильтры по виду/породе/владельцу |
| **📦 Товары** | Полный каталог товаров | Фильтры по цене/наличию/поставщику |
| **🛒 Заказы** | Все заказы с историей | Фильтры по статусу/сумме/дате |
| **🎓 Курсы** | Все курсы обучения | Фильтры по типу/цене/активности |

#### Доступные графики аналитики

| График | Данные | Тип |
|--------|--------|-----|
| **Тренды продаж** | Выручка и количество заказов по периодам | Комбинированный (линия + столбцы) |
| **Статистика пользователей** | Всего/активных/администраторов | Столбчатая диаграмма |
| **Питомцы по видам** | Распределение собак/кошек/других | Круговая диаграмма |
| **Заказы по статусам** | Ожидает/в обработке/отправлен/доставлен/отменён | Круговая диаграмма |
