...
describe('regression: префикс +7 не «съедает» национальные цифры при правке', () => {
  // Было: удаление символа из полного номера давало 10 цифр ВКЛЮЧАЯ «7» из +7,
  // и она поглощалась в номер («+7 (799) 912-34-56»). Теперь префикс снимается
  // литерально, поэтому частичный номер переформатируется без сдвига.
  it('частичный номер (9 нацифр) переформатируется без сдвига', () => {
    expect(extractPhoneDigits('+7 (999) 123-45-6')).toBe('999123456')
    expect(formatPhoneDisplay('+7 (999) 123-45-6')).toBe('+7 (999) 123-45-6')
  })

  it('национальная часть, начинающаяся с 8 (напр. код 800), сохраняется', () => {
    expect(extractPhoneDigits('+7 (800) 123-45-67')).toBe('8001234567')
    expect(normalizePhone('+7 (800) 123-45-67')).toBe('+78001234567')
  })

  it('национальная часть, начинающаяся с 7, сохраняется', () => {
    expect(extractPhoneDigits('+7 (700) 123-45-67')).toBe('7001234567')
  })
})
