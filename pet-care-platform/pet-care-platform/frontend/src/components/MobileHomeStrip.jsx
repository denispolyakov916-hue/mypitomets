...
function MobileHomeStrip() {
  return (
    ...
    >
      <div
        className={`pointer-events-auto relative flex min-h-11 w-full max-w-[min(100%,36rem)] items-center gap-2 overflow-hidden rounded-full py-1.5 pl-3 pr-0 sm:min-h-12 sm:py-2 sm:pl-3.5 sm:pr-0 ${mobileHeaderStripGlassClass}`}
        style={{ boxShadow: stripInsetShine }}
      >
        {/* Левый спейсер — уравновешивает бейдж справа: бренд держится около центра,
            но при нехватке места ужимается/обрезается, а не налезает на «Ням-коины». */}
        <span aria-hidden className="pointer-events-none invisible min-w-0 flex-1 shrink" />

        <NavLink
          to="/"
          end
          className={({ isActive }) =>
            `relative z-0 flex min-w-0 shrink items-center justify-center rounded-full px-1.5 py-0.5 transition-all active:scale-[0.98] ${
              isActive ? 'opacity-100' : 'opacity-95 hover:opacity-100'
            }`
          }
          aria-label="Питомец плюс — на главную"
        >
          <span className="truncate font-['Poppins',sans-serif] text-sm font-black leading-tight tracking-tight sm:text-[0.95rem]">
            <span className="text-primary-700">Питомец </span>
            <span className="text-accent-500">плюс</span>
          </span>
        </NavLink>

        {/* Правый блок: фикс-бейдж прижат вправо, резервирует симметричное место слева */}
        <span className="flex min-w-0 flex-1 shrink-0 justify-end pr-1 sm:pr-1.5">
          <span
            className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-gradient-to-b from-accent-300 via-accent-400 to-accent-500 py-1.5 pl-2.5 pr-4 text-xs font-semibold leading-tight tracking-tight text-primary-900 shadow-[inset_0_1px_0_rgba(255,255,255,0.42),inset_0_-1px_0_rgba(0,0,0,0.06)] sm:gap-2 sm:py-2 sm:pl-3 sm:pr-5 sm:text-sm"
