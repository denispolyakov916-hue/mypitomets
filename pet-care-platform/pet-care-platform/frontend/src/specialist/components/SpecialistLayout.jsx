...
import { BookOpen, LogOut, Menu, Video, X } from 'lucide-react';
import { useAuthStore } from '../../store/authStore';

const navItems = [
  { name: 'Курсы коррекции', href: '/specialist-panel/courses', icon: BookOpen },
];
    ...
    <div className="flex h-full flex-col bg-white">
      <div className="flex h-16 items-center border-b border-gray-200 px-4">
        ...
        <div className="ml-3 min-w-0">
          <div className="truncate text-sm font-semibold text-gray-900">Кабинет специалиста</div>
          <div className="truncate text-xs text-gray-500">Курсы коррекции поведения</div>
        </div>
      </div>
...
const SpecialistLayout = ({ children }) => {
  ...
  const handleLogout = async () => {
    await logout();
    navigate('/specialist-panel', { replace: true });
  };
        ...
        <header className="flex h-16 min-w-0 items-center justify-between gap-2 border-b border-gray-200 bg-white px-3 sm:px-4 lg:px-6">
          <div className="flex min-w-0 items-center">
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold text-gray-900">Специалист</div>
              <div className="hidden truncate text-xs text-gray-500 sm:block">Создание курсов и уроков с видео</div>
            </div>
          </div>
          <div className="flex min-w-0 flex-shrink items-center gap-2 sm:gap-3">
            <div className="min-w-0 text-right">
              ...
              <div className="hidden truncate text-xs text-gray-500 sm:block">
                {user?.role === 'course_creator' ? 'Специалист' : 'Администратор'}
              </div>
            </div>
            <button
              type="button"
