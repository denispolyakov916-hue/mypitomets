...
function Input({ value, onChange, type = 'text', placeholder, ...props }) {
  return (
    <input
      ...
      className="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
      {...props}
    />
  )
}
    ...
    <div className="space-y-5">
      ...
      <Card title="Основная информация">
        <Field label="Название курса" required>
          <Input value={values.title} onChange={set('title')} placeholder="Например: Базовые команды для собак" />
        </Field>
      ...
      {/* Pricing & Duration */}
      <Card title="Стоимость и длительность">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Field label="Цена (руб)" hint="0 = бесплатно">
            <Input type="number" min="0" step="0.01" value={values.price} onChange={(v) => set('price')(parseFloat(v) || 0)} placeholder="0" />
          </Field>

          <Field label="Длительность (мин)">
            <Input type="number" min="1" value={values.duration} onChange={(v) => set('duration')(parseInt(v) || 60)} placeholder="60" />
          </Field>

          <Field label="Время прохождения" hint="Например: 2-3 недели">
            <Input value={values.completion_time} onChange={set('completion_time')} placeholder="2-3 недели" />
          </Field>
        </div>
      </Card>

      {/* Instructor */}
      <Card title="Инструктор">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Имя инструктора">
            <Input value={values.instructor_name} onChange={set('instructor_name')} placeholder="Анна Петрова" />
          </Field>
